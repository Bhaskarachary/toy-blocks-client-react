...
const Node = ({ node, expanded, toggleNodeExpanded }) => {
  ...
  let blocks = [];
  node.data && node.data.map((element, i) => {
    blocks.push(
      <div className={classes.grey} key={i}>
        <div className={classes.blue}>{element.id.toString().padStart(3, '0')}</div>
    <div>{element.attributes.hash}</div>
      </div>
    )
  });
  return (
